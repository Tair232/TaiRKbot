...
Проект специально сделан без папки `src`: `main.js` и `style.css` лежат прямо в корне репозитория.

Загрузить весь проект в GitHub и подключить репозиторий к Bothost.
...
Bothost сам передаёт токен бота через свою системную переменную, поэтому отдельно дублировать токен в ENV обычно не нужно. Код также понимает `DISCORD_TOKEN`, `DISCORD_BOT_TOKEN` и `BOT_TOKEN`.
